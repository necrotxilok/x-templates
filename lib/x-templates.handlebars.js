...
(function() {
	...
	function defineTemplateFn(expression) {
		if (templateDefs[expression]) {
			var fn = templateDefs[expression];
		} else {
			...
			try {
				...
				throw new Error('Unable to process expression [' + expression + ']. ' + e.message + '.');
			}
			templateDefs[expression] = fn;
		}
		return fn;
	}

	function defineExpressionFn(expression) {
		var parsedExpression = prepareExpression(expression);
		if (expressionDefs[expression]) {
			var fn = expressionDefs[expression];
		} else {
			try {
				var def = `function fn(data) {
					try {
	...
	function getParsedBlocks(templateName, parsedStrings, html) {
		...
		for (var i = 0; i < parsedStrings.length; i++) {
			...
			if (part.startsWith('{{>')) {
				var expression = part.replace('{{>', '').replace('}}', '').trim();
				try {
					var fn = defineTemplateFn(expression);
				} catch(e) {
			...
			if (part.startsWith('{{')) {
				var expression = part.replace('{{', '').replace('}}', '').trim();
				try {
					var fn = defineExpressionFn(expression);
				} catch(e) {
					setSyntaxError(e.message);
					break;
				}
	...
	Handlebars.registerBlockFunction('each', function(params, block) {
		...
		var eachValues = [];
		try {
			var fn = defineExpressionFn(loopVar);
			eachValues = fn(data);
		} catch(e) {
			logRenderError(e.message, block);
		}			
