(function() {
	...
	function logRenderError(message, block, data, pos, part) {
		var renderErrorMessage = message + '\nCheck "' + block.template + '" template.';
		if (pos) {
			renderErrorMessage += ' Content position ' + pos + '.';
		}
		if (part) {
			renderErrorMessage += '  Near:\n' + part;
		}
	...
	function calcExpression(expression, data) {
		...
		try {
			...
			console.error(e);
			throw new Error('Unable to process expression [' + expression + '].');
		}
		return false;
	}
			...
			if (type == 'text') {
				...
				if (part.startsWith('{{>')) {
					...
					try {
						...
						logRenderError(e.message, block, data, i, part);
					}
					debugLog(part, '- Render Template:', templateName);
	...
	function execBlockFunction(block, data) {
		...
		var blockHelper = $.extend({}, block, {
			renderContents: function() {
				debugLog('Render Block Content with', data);
				result = renderBlockContents(block.contents, data, block);
				return result.join('');
			},
			renderInverse: function() {
				debugLog('Render Block Inverse with', data);
	...
	var Handlebars = {
		...
		blockFunction: function(fn, action) {
	...
	Handlebars.blockFunction('if', function(expression, block) {
		...
		try {
			...
			logRenderError(e.message, block, data);
		}			
		if (result) {
	...
	Handlebars.blockFunction('each', function(expression, block) {
	...
	Handlebars.blockFunction('assign', function(expression, block) {
		debugLog('assign', expression, block);
		return '';
	});
	Handlebars.blockFunction('capture', function(expression, block) {
