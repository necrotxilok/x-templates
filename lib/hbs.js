	...
	function calcExpression(expression, data) {
		...
		for (var i = 0; i < fnDefs.length; i++) {
			...
			if (typeof fnDef == 'function') {
				currentDef += "var " + fnName + "=" + fnDef + ";";
			} else {
				currentDef += "var " + fnName + "=<invalid_data_type>;";
			}
			try {
				var checkFn = eval(currentDef);
			} catch(e) {
				if (HANDLEBARS_DEBUG_MODE) {
					throw new Error('ERROR: Unable to define [' + fnName + '] from helper functions to calc expression [' + expression + ']. \nMESSAGE: ' + e.message + '.');
				}
			}
			defFunctions += "    " + currentDef + "\n";
		}
		// Execute Expression
		try {
			result = eval(defFunctions + "\n" + defData + "\n" + expression);
		...
		for (var i = 0; i < contents.length; i++) {
			...
			if (type == 'text') {
				...
				if (part.startsWith('{{>')) {
					...
					debugLog('[ Render Template ' + templateName + ' ]');
				...
				if (part.startsWith('{{')) {
					...
					debugLog('[ Render value ' + part + ' = ' + value + ' ]');
